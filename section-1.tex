...
\begin{lemma}[Simple Calculations]\hfill
    \begin{enumerate}
        ...
        \item Again, applying the product rule gives
              ...
              \]
        \item Let \( f \) be differentiable at \( x_0 \). From the definition follows that \( f \)
              is \emph{Lipschitz continous}, e.g. there exists a \( L > 0 \), so that
              \( |f(x) - f(x_0)| < L|x - x_0| \) in some small enough environment of \( x_0 \). Furthermote
              if there exists a sequence \( x_n \to x_0 \) with \( f(x_n) = f(x_0) \) then \( f'(x) = 0\).
        \item The chain rule: \( (f \circ g)' = (f' \circ g)\, g' \). For \( g(x) \ne g(x_0) \) follows
              \[
                  \frac{f(g(x)) - f(g(x_0))}{x - x_0} =
                  \frac{f(g(x)) - f(g(x_0))}{g(x) - g(x_0)} \frac{g(x) - g(x_0)}{x - x_0}
              \]
              In case there exists a sequence \( x_n \to x_0 \) with \( g(x_n) = g(x_0) \) the remarks above give
              \( (f \circ g)'(x_0) = 0 \).
