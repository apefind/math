...
\begin{lemma}[Simple Calculations]\hfill
    \begin{enumerate}
        ...
        \item Let \( f \) be differentiable at \( x_0 \) and  \( x_n \to x_0 \) with \( f(x_n) = f(x_0) \).
              Then \( f'(x) = 0 \). Consequently, if \( f'(x) \ne 0 \) then \( f(x) \ne f(x_0) \)
              in some local environment of \( x_0 \).
        \item The chain rule: \( (f \circ g)' = (f' \circ g)\, g' \). For \( g'(x_0) \ne 0 \) there exists
              a local environment with \( g(x) \ne g(x_0) \) and
              \[
