...
\begin{lemma}[Substitution for Integrals]
...
\begin{examples}\hfill
    \begin{enumerate}
		...
		\item Consider \( \varphi(x) = \sin(x) \) where \( \varphi(0) = 0 \) and \( \varphi(\pi / 2) = 1 \).
			...
			\]
		\item Let \(f \in C[a,b] \) and \( \varphi(x) = (1 - t)a + bt \). Then
			\[
				\int_a^b f(x)\,dx = (b - a)\int_0^1 f((1 - t)a + bt)\,dt
			\]
		\item Let \(f(x) = x^n \) and \( \varphi(x) = t^m \). Then as expected
			\[
				\int_0^1 x^n\,dx
					= m\int_0^1 t^{nm} t^{m - 1}\,dt
