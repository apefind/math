...
\begin{definition}[Error Function]
    Let \( \alpha : \in C^1(\R) \) be an activation function, \( w \in \Rn\) and \( b \in \R \).
    For a finite \emph{test set} \( T \subset \Rn \times \R \) define the \emph{error function}
    \[
        E(w,b) = \frac{1}{2} \sum_{(x,y) \in T} (y - \alpha(wx + b ))^2
    \]
\end{definition}
\bigskip
...
\begin{lemma}
    ...
    \[
        \begin{split}
            \frac{\partial E}{\partial w_i}(w, b) &= \sum_{(x,y) \in T} -(y - \alpha(wx + b)) \alpha'(wx + b) x_i \\
            \frac{\partial E}{\partial b}(w, b) &= \sum_{(x,y) \in T} -(y - \alpha(wx + b)) \alpha'(wx + b)
        \end{split}
    \]
\end{lemma}
