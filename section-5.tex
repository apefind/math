...
\end{algorithm}
\inputminted[fontsize=\small, framesep=0.35cm, frame=lines, python3=true]{python}{notebook/neural_network/weight.py}
\bigskip
...
\end{algorithm}
\inputminted[fontsize=\small, framesep=0.35cm, frame=lines, python3=true]{python}{notebook/neural_network/perceptron.py}
\bigskip
...
\inputminted[fontsize=\small, framesep=0.35cm, frame=lines, python3=true]{python}{notebook/neural_network/delta_rule.py}
\bigskip
...
\begin{definition}[Feedforward Neural Network]
    \hfill
    \begin{enumerate}
        % \item Let \( W^0 \in \Rnm \), \( W^1 \dots W^N \in \Rmm \) and \( W^{N + 1} \in \R^{m \times l} \)
        %       be \emph{weight matrices}. Furthermore let \( b^0 \in \R^m \), \( b^1 \dots b^N \in \Rm \)
        %       and \( b^{N + 1} \in \R^l \) be \emph{biases}.
        \item Let \( W^0 \in \Rnm \), \( W^1 \dots W^N \in \Rmm \) and
              \( W^{N + 1} \in \R^{m \times l} \) as well as \( b^0 \in \R^m \), \( b^1 \dots b^N \in \Rm \)
              and \( b^{N + 1} \in \R^l \). Then \( W = (W^0, W^1, \dots W^{N + 1} ) \) and
              \( B = (b^0, b^1, \dots b^{N + 1}) \)  are the \emph{weights} and \emph{biases}.
        \item For an activation function \( \alpha \) and \( x \in \Rn \) define \( y^0 = \alpha(W^0x + b^0) \) and
              \[
                  y^k = \alpha(W^k y^{k - 1} + b^k)
              \]
        \item Then \( f: \Rn \to \R^l \) defined as
              \[
                  f(x) = y^{N + 1} % = \alpha(W^{N + 1} y^N + b^{N + 1})
              \]
              is called a \emph{feedforward neural network of \( N \) hidden layers}.
        \item The \emph{error function} is defined as
              \[
                  E(W, B) = \frac{1}{2} \sum_{x, y \in T} {\| y - f(x) \|}^2
              \]
...
\begin{remarks}
    \hfill
    \begin{enumerate}
        \item Assume \( W^k = (w^k_{ij}) \) and \( b^k = (b^k_i) \) are the entries of the matrices and biasis.
              Then
              \[
                  y^k_{j} = \alpha (\sum_{i} w_{ij} y^{k - 1}_i + b^k_j)
              \]
