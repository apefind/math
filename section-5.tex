...
\begin{examples}
    ...
    \begin{enumerate}
        \item The Delta Rule does not always converge to a solution. A counter example is the boolean \emph{NAND}
              operator together with \emph{ReLU} as activation function. Starting with positive weights
...
\begin{enumerate}
    ...
    \item Furthermore \( f^k(x) = x^k \) where the dimensions are defined by the respective weights and biases.
    \item Then \( f^k = \alpha^k(Wx + B) \) is used as a mapping with respect to weights and biases.
          %   \item Furthermore \( f^k(x) = \alpha^k(Wx + B) = x^k \) within the given dimensions of the
          %   weights and biases.
          %   Furthermore \( f_j = \alpha_j(Wx + B) \) for \( f = (f_1, f_2, \dots, f_l) \).
\end{enumerate}
\bigskip
