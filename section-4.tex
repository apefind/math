...
As an algorithm, start off with \( w_0 = 0 \) and define
\[
    w_{k + 1} = \left \{
    \begin{array}{ll}
        w_k + x & \text{ if } w_k x < 0 \\
        w_k + y & \text{ if } w_k y < 0 \\
    \end{array}
    \right.
\]

Assume \( w^*\in \Rn \) with \( \|w^*\| = 1\) separates \(x \) and \( y\).
Let \( \|x\| \le \|y\| < r \). Then
\[
    w^*w_k = w^*w_{k - 1} + w^*x > w^*w_{k - 1} + \delta > \cdots > k\delta
\]
And
\[
    \|w_k\|^2 = \|w_{k - 1} + x\|^2 = \|w_{k - 1}\|^2 + 2w_{k - 1}x + \|x\|^2
    \le \|w_{k - 1}\|^2 + r^2 \le \cdots \le kr^2
\]
Thus
\[
    k^2\delta^2 \le \|w_k\|^2 \le kr^2
\]
So \( k \) is limited by the following inequality
\[
...
\begin{theorem}[Perceptron Convergence Theorem]
    Let \( X = M \cup N \) be separable with \( w^*\in \Rn \).
    Define \( w_0 = 0 \) and iterate over \( x \in X \) via
    \[
        w_{k + 1} = \left \{
        \begin{array}{ll}
            w_k + x & \text{ if } x \in N \text{ and } w_k x \le 0 \\
            w_k - x & \text{ if } x \in M \text{ and } w_k x \ge 0 \\
            w_k     & \text{ else }                                \\
        \end{array}
        \right.
    \]
    until no more changes occurr.  Suppose \( w^*x > \delta > 0 \) and \( \|x\| < r \) for \( x \in X \).
    Then the algorithm stops after at most 
    \[
        k\le \frac{r^2}{\delta^2}
    \]
    steps
...
\begin{proof}
    ...
    \[
        w^*w_k = w^*w_{k - 1} + w^*x > w^*w_{k - 1} + \delta > \cdots > k\delta
    \]
    For \( w_{k - 1} x < 0 \) it is
    \[
        \|w_k\|^2 = \|w_{k - 1} + x\|^2 = \|w_{k - 1}\|^2 + 2w_{k - 1}x + \|x\|^2
        \le \|w_{k - 1}\|^2 + r^2 \le \cdots \le kr^2
    \]
    Without loss of generality assume \( \|w^*\| = 1\). Then
    \[
        k^2\delta^2 \le \|w_k\|^2 \le kr^2
    \]
    So \( k \) is limited by the following inequality
