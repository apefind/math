...
The actual theshold is given by the following lemma.

\begin{lemma}
    Let \( x, y \in \Rn \) with \( \|x\| < \|y\| \) and \( xy = \|x\| \|y\| \cos\alpha < 0\). If 
    \[
        \|x\| > -\|y\|\cos\alpha 
    \]
    then \( x(x + y) = \|x\| \|x + y\| \cos\beta> 0 \).
\end{lemma}

\begin{proof}
It is 
\[
    x(x + y) = xx + xy = {\|x\|}^2 + \|x\| \|y\| \cos\alpha = \|x\| \|x + y\| \cos\beta
\]
and thus
\[
