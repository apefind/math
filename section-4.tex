...
While this seems reasonable, it is unclear whether the algorithm always yields a result 
...
\begin{examples}
    ...
    \begin{enumerate}
        ...
        \item
              Let \( w = x / \|x\| + y/ \|y\| \). Then
              \[
                  wx = \frac{{\|x\|}^2}{\|x\|} + \frac{yx}{\|y\|} = \|x\| + \|x\|\cos\alpha = (1 + \cos\alpha)\|x\|
              \]
              ...
              \]
              and thus \( \beta = \gamma \)
        \item For any given integer \( k \) find two vectors so that more than \( k \) steps are needed
    \end{enumerate}
\end{examples}
\bigskip
...
% The more interesting task to find a weight that satisfies \( wx > 0 \) for \( x \in M \)
% and \( wx < 0 \) for \( x \in N \). The question of convergence is answered 
% by the \hyperref[thm:perceptron_convergence]{Perceptron Convergence Theorem~\ref*{thm:perceptron_convergence}}.
% The proof estimates the change of angle against the change of lengths.
...
\begin{proof}
    ...
    The same estimations hold for \( x \in N \) with \( w_k x \ge 0 \) and induction yields
    \[
